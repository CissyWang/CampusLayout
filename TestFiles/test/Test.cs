...
namespace User
{
    class Program : IApp
    {
        ...
        Campus campus; 
        int resultCount = 7; //解数
        int grid = 20;             //运行单元大小
        bool info=false;
        #endregion
        string mustFile = "../../../南工职大/mustBuilding.csv";
        string optionalFile = "../../../南工职大/optionalBuilding1.csv";
        string exportPath = "../../../南工职大/districtExport.csv";

        string siteCsv = "../../../test/site1.csv";//场地信息
        string districtCsv = "../../../test/districtExportE1.csv";//分区信息
        string locationCsv = "../../../test/district_location.csv";//分区位置
        double time = 20.0;  
        ...
        public void SetUp()
        {
            ...
            myCal.PoolSearchMode = 0;// （0专注最优解，1按数量搜索可行解并不保证质量，2系统搜索可行解）
            myCal.Time = time;
            ...
            myCal.DistrictLink(4, 18, true);

            //for(int i = 0; i < 4; i++)
            //{
            //    myCal.PointLink(i, 0, "1");
            //}
            //for (int i = 4; i < 7; i++)
            //{
            ...
            myCal.Core_Inside("all", true,true);
        ...
        public void ShowResult()
        {
            ...
            //{
            //    //if (entrs.IndexOf(poi) < 2)
            //    //{
            //        PushMatrix();
            //        //TextSize(grid);
            //        Translate(grid * poi.p, grid * poi.q);
            //        TextAlign(0, 1);
            //        Text("P" + entrs.IndexOf(poi).ToString(), 0, 0, 0);
            //        Sphere(0.3f * grid);
            //        PopMatrix();
            //    //}
            //}
            ...
            foreach (DistrictVar dv in myCal.DistrictVars)
            {
                ...
                if (info)
                {
                    double buildingArea = dv.Area(resultN) * dv.District.Building_area / dv.District.Result_area(resultN);
                    double siteArea = Math.Round(dv.Area(resultN), 0);
                    buildingArea = Math.Round(buildingArea, 0);
                    TextSize(0.8f * grid);
                    double temp = Math.Round(0.05f * 0.05 * siteArea, 1);
                    Text("("+index + ")" +temp.ToString(), grid * rect.Center.p, grid * (rect.Center.q ), 0);
