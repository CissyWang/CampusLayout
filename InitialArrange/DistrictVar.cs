...
namespace InitialArrange
{
    public class DistrictVar
    {
        ...
        internal List<int[]> roadSide = new List<int[]>();
        internal List <int> sigleP_link = new List<int>();
        internal List<int> road_link = new List<int>();
        internal List<int> road_align = new List<int>();
        internal double align_tolerance;

        internal Domain area_lim;

        public IDistrict District { get => district; }
  

        #region Constructor
        public DistrictVar(IDistrict district, Site site, int isInteger)
        {
        ...
        //用于Group\Core
        public DistrictVar(Site site, int isInteger, double lenToWidth)
        {
            this.site = site;
            this.isInteger = isInteger;
        ...
        public DistrictVar( Site site,Domain[] domain, int isInteger, double lenToWidth)
         {
            this.site = site;
            this.isInteger = isInteger;
            this.lenToWidth = lenToWidth;
        ...
        public void SetVar(GRBModel model)
        {
            ...
            if (district!=null&&district.Length_min > 0)
            {
                model.AddConstr(dx >= district.Length_min, "");
                model.AddConstr(dy >= district.Length_min, "");
            }
        ...
        ///*设置互不重叠*
        internal void ConstrOverlap(GRBModel model, List<DistrictVar> districtVars, double spacing)
        {
        ...
        internal void SetRoadConstr(GRBModel model)
        {
            ...
            foreach (int i in road_align)
            {
                ...
                double b = site.Roads[i].B;
                double c = site.Roads[i].C;
                double t = Math.Sqrt(a * a + b * b)*align_tolerance;
                ...
                model.AddConstr((a * (x + dx / 2) + b * (y - dy / 2) + c) + (1 - bool_align[2]) * site.w >= -width / 2 * t, "");
                model.AddConstr((a * (x + dx / 2) + b * (y - dy / 2) + c) - (1 - bool_align[2]) * site.w <= width / 2 * t, "");

                model.AddConstr((a * (x - dx / 2) + b * (y - dy / 2) + c) + (1 - bool_align[3]) * site.w >= -width / 2 * t, "");
                model.AddConstr((a * (x - dx / 2) + b * (y - dy / 2) + c) - (1 - bool_align[3]) * site.w <= width / 2 * t, "");
        ...
        //分区之间靠近
        internal void DistrictLinkObj(GRBQuadExpr obj,DistrictVar dv)
        {
        ...
        internal void DistrictDistObj(GRBQuadExpr obj, DistrictVar dv)
        {
            double t = 100 / (site.w * site.w + site.h * site.h);
        ...
        internal void SetResult(GRBModel model,int result)
        {
            ...
            //if (district != null)
            //{
            //    Console.WriteLine(district.name + "_results" + result + ":  (" + xr + "," + yr + ")" + "," + dxr + "," + dyr);
            //}
            rectResults.Add(rect);
        }

        internal void inOrOutOthers(GRBModel model,DistrictVar other,double width,InOrOut inside,bool align)
        {
            ...
            model.AddConstr( x +  dx / 2 - (1 - inOut[0]) * site.w <= other.x - other.dx / 2 -  width / 2, "");
            model.AddConstr( x -  dx / 2 + (1 - inOut[1]) * site.w >= other.x + other.dx / 2 +  width / 2, "");
            model.AddConstr( y +  dy / 2 - (1 - inOut[2]) * site.h <= other.y - other.dy / 2 -  width / 2, "");
            model.AddConstr( y -  dy / 2 + (1 - inOut[3]) * site.h >= other.y + other.dy / 2 +  width / 2, "");

            model.AddConstr( x +  dx / 2 - (1 - inOut[4]) * site.w <= other.x + other.dx / 2 -  width / 2, "");
            model.AddConstr( x -  dx / 2 + (1 - inOut[4]) * site.w >= other.x - other.dx / 2 +  width / 2, "");
            model.AddConstr( y +  dy / 2 - (1 - inOut[4]) * site.h <= other.y + other.dy / 2 -  width / 2, "");
            model.AddConstr( y -  dy / 2 + (1 - inOut[4]) * site.h >= other.y - other.dy / 2 +  width / 2, "");
            ...
            if (inside == InOrOut.Inside) { 
                ...
                if (align)
                {
                    ...
                    model.AddConstr(x + dx / 2 + (1 - isAlign[0]) * site.w >= other.x + other.dx / 2 - width / 2, "");
                    model.AddConstr(x - dx / 2 - (1 - isAlign[1]) * site.w <= other.x - other.dx / 2 + width / 2, "");
                    model.AddConstr(y + dy / 2 + (1 - isAlign[2]) * site.h >= other.y + other.dy / 2 - width / 2, "");
                    model.AddConstr(y - dy / 2 - (1 - isAlign[3]) * site.h <= other.y - other.dy / 2 + width / 2, "");
            ...
            else if (inside== InOrOut.Outside)
            {
                ...
                if (align)
                {
                    model.AddConstr(x + dx / 2 >= other.x - other.dx / 2 - width / 2, "");
                    model.AddConstr(x - dx / 2 <= other.x + other.dx / 2 + width / 2, "");
                    model.AddConstr(y + dy / 2 >= other.y - other.dy / 2 - width / 2, "");
                    model.AddConstr(y - dy / 2 <= other.y + other.dy / 2 +width / 2, "");
                }
            }
        }
        ...
        public int Index
        {
            ...
            {
                int index = 0;
                foreach (DistrictVar dv in District.DistrictVars)
                {
                    if (dv == this)
                        break;
                    else
        ...
        public double Area(int resultN,int grid)
        {
            return grid*grid*area[resultN];
        }
        ...
        public double BuildingArea(int resultN)
        {
            ...
            if (district != null)
            {
                buildingArea = area[resultN] * district.grid * district.grid * District.Building_area / District.Result_area(resultN);
            }
            return buildingArea;
        }
        ...
        {
           return area[resultN] * district.grid * district.grid / District.Result_area(resultN);
        }

        public double floor_Area(int resultN)
        {
            return Ratio(resultN) * district.Buildings.FloorArea_all();
        }  
            ...
            for (int n = 0; n < resultCount; n++)
            {
                try
                {
                    dataStr += $",({rectResults[n].X1};{rectResults[n].Y1};" +
                        ...
                        $"{Math.Round(Area(n, district.grid), 2)};{Math.Round(BuildingArea(n), 2)}";
                    if (BuildingArea(n) > 0&&district.Buildings!=null)
                    {
